...
CREATE TABLE subscriptions (
                               ...
                               status            VARCHAR(20) NOT NULL DEFAULT 'ACTIVE',  -- ACTIVE | UNDER_REVIEW | CANCELLED
                               FOREIGN KEY (team_id) REFERENCES teams(team_id),
...
-- High-volume event log: every login/usage ping for a tool by a user
...
-- per-tool scans; including user_id + event_date lets the idle-
-- detection query be answered entirely from the index.
CREATE INDEX idx_usage_sub_user_date
    ON usage_events (subscription_id, user_id, event_date DESC);

CREATE INDEX idx_subscriptions_team_tool
...
-- ---------------------------------------------------------------------
-- 4. Trigger — auto-flag a subscription UNDER_REVIEW once its last
--    remaining seat is removed
-- ---------------------------------------------------------------------
